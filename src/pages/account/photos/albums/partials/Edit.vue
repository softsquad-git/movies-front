<template>
  <q-dialog v-model="isShow" persistent>
    <q-card>
      <div class="text-h6 p-15">{{ title }}</div>
...
export default {
  ...
  methods: {
    save() {
      this.$axios.post(`user/albums/update/${this.itemId}`, this.data)
      .then((data) => {
    ...
    openModal(item) {
      ...
      this.data.is_private = item.is_private;
      this.data.is_visibility = item.is_visibility;
      this.isShow = true;
    }
  }
}
